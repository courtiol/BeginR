...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Working on the language}
\verb`substitute` is an expression (a piece of code) that is \textit{not evaluated}.
\verb`eval` can be used to evaluate the piece of code.
<<>>=
saved_code <- substitute(a + b)
saved_code
...
@
\vfill
Note: This can be useful to evaluate the same code in different environments.
\end{frame}
