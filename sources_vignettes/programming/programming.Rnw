...
\pause
\vfill
Inline shortcut (useful in \verb`*apply()`):
<<>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\r \ has the same numerical issues as most programming languages}
...
<<figLargenumbers, fig.width=6, fig.height=6, include=F>>=
plot(   (2^30 + 1:10) - 2^30, type = "o", col = 1, lwd = 2, ylim = c(0, 20), ylab = "Difference")
points( (2^53 + 1:10) - 2^53, type = "o", col = 2, lwd = 2)
points( (2^55 + 1:10) - 2^55, type = "o", col = 3, lwd = 2)
points( (2^56 + 1:10) - 2^56, type = "o", col = 4, lwd = 2)
legend("topleft", bty = "n", col = 1:4, legend = c("(2^30 + 1:10) - 2^30",
                                         "(2^53 + 1:10) - 2^53",
                                         "(2^55 + 1:10) - 2^55",
                                         "(2^56 + 1:10) - 2^56"), lty = 1, lwd = 2, pch = 1)
@
\begin{center}
\includegraphics[height=0.6\textheight]{./figure/figLargenumbers-1.pdf}
\end{center}
\pause
<<>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{\r \ has the same numerical issues as most programming languages}
Solution: beware of \texttt{double}s and use adequate functions, not boolean tests, when performing comparisons!
<<eval=T>>=
??"equality"
@
\begin{scriptsize}
...
all_pairwise2 <- function(proba, groups){
  ...
  return(data.frame(group1 = to_do[, 1], group2 = to_do[, 2], OR = OR))
}
...
\item assuming that the null hypothesis is true and that you have sampled 20 males and 20 females, what is the probability of false positives for the threshold alpha = 0.05? (And for any threshold between 0 and 1\%?)
\end{itemize}
\end{minipage}
\end{center}
\end{frame}
...
\item how to code
\item new functions or packages that could be useful for you
\end{itemize}
\end{minipage}
\end{center}
\end{frame}
...
Start by simply typing the function name without brackets and here with the namespace as prefix to avoid loading it:
\vspace{1cm}
<<size="scriptsize">>==
mosaic::oddsRatio
@
...
Sometimes, the code is not directly displayed\dots e.g. \r \  methods (\texttt{S3}):
\vspace{1em}
<<size="scriptsize">>==
residuals
@
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{How to get the code behind a function?}
Add the \verb`class` at the end of the function name to get the code for exported \r \ (\texttt{S3}) methods:
\vspace{1em}
<<size="scriptsize">>==
residuals.lm
@
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{How to get the code behind a function?}
It is also possible to get the code of non-exported \r \ methods (\texttt{S3}):
\vspace{1em}
<<size="scriptsize">>=
residuals.nls
@
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Working on the language}
\verb`quote` \& \verb`substitute` generate unevaluated pieces of code.

\verb`eval` can be used to evaluate the piece of code.
<<>>=
...
@
\vfill
Note: the difference between \verb`quote` \& \verb`substitute` is that \verb`substitute` allows for substitution before the evaluation (useful to play with variable names).
\end{frame}
...
\begin{itemize}
\item \texttt{S3} (legacy from S version 3, in \emph{base})
\item \texttt{S4} (legacy from S version 4, in core package \emph{methods})
...
\begin{itemize}
\item \texttt{R6} (in the package \texttt{R6}, one of the most downloaded package on CRAN!)
...
\begin{itemize}
\item suitable for data that evolve over time (modularity and re-usability)
\item \texttt{RC}, \texttt{R6}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{A simple example using \texttt{S3}}
<<>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{A simple example using \texttt{S4}}
<<>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{A simple example using \texttt{RC}}
<<>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{A simple example using \texttt{R6}}
<<>>=
library(R6)
R6_obj <- R6Class(
  public  = list(
...
\texttt{S3} objects are (generally) not mutable (same for \texttt{S4}):
\begin{columns}[t]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Encapsulated OOP requires mutability}
\texttt{R6} objects are mutable (same for \texttt{RC}):
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Functional way using \texttt{S3}: functions}
<<>>=
death <- function(pop) {
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Functional way using \texttt{S3}: run}
<<>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{OOP way using \texttt{R6}: definition of the class \texttt{individual}}
<<>>=
individual <- R6Class(
  public  = list(
    age = NA,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{OOP way using \texttt{R6}: test}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{OOP way using \texttt{R6}: definition of the class \texttt{population}}
<<size = "scriptsize">>=
population <- R6Class(
  public  = list(
    individuals = list(),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{OOP way using \texttt{R6}: run}
<<>>=
pop <- population$new()
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{OOP way using \texttt{R6}: re-definition of the class \texttt{individual}}
<<size = "scriptsize">>=
individual <- R6Class(
  public  = list(
    age = NA,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{OOP way using \texttt{R6}: re-run}
<<>>=
pop <- population$new()
for (i in 1:50) pop$year()
pop$count()
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Functional way using \texttt{S3}: re-defining functions}
We would have to recode everything \dots
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Pros and cons of \texttt{R6} for IBM}
...
Cons
\begin{itemize}
  \item initially difficult for those knowing mostly \texttt{S3}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{A more advanced simulation model using \texttt{R6}}
<<eval = FALSE>>=
?Butterfly
@
\end{frame}
