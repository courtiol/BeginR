...
\item \verb`ggplot2` is part of the \texttt{tidyverse} universe (from RStudio)
\item we will focus on traditional graphics and \verb`ggplot2` (\verb`lattice` is excellent but a bit more difficult)
...
@
\column[b]{0.32\linewidth}
<<ggplot_eg, out.width = "0.7\\linewidth", out.height = "0.7\\linewidth", message = FALSE>>=
...
% mind that using only<>{} prevents to correctly display dollar signs, and tildes...
<<scatter_base_2a>>=
...
@
\vfill
Note: you can use numbers to describe basic colours.
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Traditional graphics: scatter plots}
For some point shapes you can set both an outline colour (col) and background colour (bg):
\begin{columns}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Traditional graphics: scatter plots}
You can add a legend to the plot:
<<scatter_base_5>>=
palette(c("red", "blue", "green"))
...
points(Petal.Length ~ Petal.Width, data = setosa, pch = 21, bg = "red")
@
\vfill
...
<<scatter_base_9, out.height="0.3\\linewidth", out.width="0.3\\linewidth", message = FALSE>>=
...
par(oldpar) ## restore default values
@
\vfill
...
\vfill
\pause
Challenge: have a look at the relationship between the happiness and the price of present using the dataset \texttt{Surprise}.
\end{frame}
...
You can use weird characters:%png device below to render the unicode!
<<aes_8, out.width = "0.3\\linewidth", out.height = "0.3\\linewidth", dev='png'>>=
foo <- round(pi, 3)
plot(Petal.Length ~ Petal.Width, data = iris,
     xlab = expression(beta^2 + 2[alpha]), ylab = expression(paste(italic("bla bla bla"))),
     main = paste("Plot number", foo))
text(0.4, 4, "\u2640", cex = 3) ## requires font with unicode installed (try it!)
text(0.6, 4, "\\VE", vfont = c("serif", "plain"), cex = 3)
@
\vfill
Note: unicode characters are nice but source of problems. How to make it work depends on both the OS, graphical device and typefaces installed on your computer.
\end{frame}
...
\begin{center}
  \includegraphics[height=4cm, keepaspectratio]{../figures/plot10}
  \hspace{0.1cm}
  \includegraphics[height=4cm, keepaspectratio]{../figures/plot5}
  \hspace{0.1cm}\\
  \includegraphics[height=4cm, keepaspectratio]{../figures/plot9}
  \hspace{0.1cm}
  \includegraphics[height=4cm, keepaspectratio]{../figures/plot8}
\end{center}
\end{frame}
...
\begin{center}
\large{Use the datasets called \texttt{population\_UK} and \texttt{deaths\_UK} to compute yearly mortality rates for individuals below 15 yrs only and make a really nice plot of the outcome!}
\end{center}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Possible solution}
<<children, echo = FALSE, message = FALSE, dev='cairo_pdf', out.width="0.55\\linewidth", fig.width=7, out.height="0.5\\linewidth", fig.height=6.5>>=
deaths_UK %>%
...
full_join(deaths_processed, pop_processed) %>%
  mutate(rel_Deaths = 1000*tot_Deaths/tot_Pop) -> data_processed

par(mgp = c(3.2, 1, 0), mar = c(5, 5, 1, 1))

plot(rel_Deaths ~ Year, data = data_processed, col = NULL, axes = FALSE,
     ylab = "Mortality rate (\u2030)", xlab = "Years",
     main = paste0("Children mortality (", min(data_processed$Year), "-", max(data_processed$Year), ")"))

abline(v = seq(min(data_processed$Year), max(data_processed$Year), by = 1), col = "lightgrey", lwd = 0.5)
abline(h = 0:ceiling(max(data_processed$rel_Deaths)), col = "lightgrey", lwd = 0.5)

polygon(x = c(data_processed$Year, rev(x = data_processed$Year)),
        y = c(data_processed$rel_Deaths, rep(x = 0, times = nrow(data_processed))),
        col = "darkgreen")

axis(side = 1, at = seq(min(data_processed$Year), max(data_processed$Year), by = 5), las = 2)
axis(side = 2, las = 1)
box()
@
\end{frame}
...
<<knitr options 2, echo = FALSE, message = FALSE>>=
opts_chunk$set("size" = "scriptsize",
               "fig.width" = 6,
               "fig.height" = 5,
...
\section{Plotting with \texttt{ggplot2}}
\subsection{introduction}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{\texttt{ggplot2}: introduction}
...
\justifying
``\emph{This book [...] presents a unique foundation for producing almost every quantitative graphic found in scientific journals, newspapers, statistical packages, and data visualization systems.}''
\vfill
\pause
In Wickham's words:\\
``\emph{The grammar of graphics is an answer to a question: what is a statistical graphic? [\dots] In brief, the grammar tells us that a statistical graphic is a
mapping from data to aesthetic attributes (colour, shape, size) of geometric objects (points, lines, bars). The plot may also contain statistical transformations of the data and is drawn on a specific coordinate system. Facetting can be used to generate the same plot for different subsets of the dataset. It is the combination of these independent components that make up a graphic.}'' (Wickham ggplot2 book)
\vfill
\pause
In plain english, that means that the graphics are build by considering building blocs or layers. The originality is that the user directly handles different functions corresponding to each layer and can combine them freely.
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{How to learn \texttt{ggplot2} on your own?}
\begin{itemize}
\item forum: \url{https://community.rstudio.com}
\item reference:\url{http://ggplot2.tidyverse.org/reference/}
\item the RStudio cheatsheet
\item the books:
\end{itemize}
\begin{center}
\includegraphics[height=4.5cm]{../figures/Wickham_ggplot2.jpg}
\hspace{1cm}
\includegraphics[height=4.5cm]{../figures/Chang_RGraphics.jpg}
\end{center}
\vfill
Note: Wickham's books (or their sources) are free and open source!
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: introduction}
Overall structure of a \texttt{ggplot2} call:
<<scatter_gg_2, fig.width = 4, fig.height = 4, out.width = "0.3\\linewidth", out.height = "0.3\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length)) + ## we specify the source of data and map the variables
  ...
  scale_x_continuous(limits = c(0, 5)) + ## we can fiddle with the scale_*_* functions to adjust the axes, colouration etc.
  theme(axis.title = element_text(size = 22)) # we can use one of the theme_* functions to change things like legends, font size etc.
@
\vfill
Note: to better understand, execute the code progressively adding one command at a time!
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: scatter plots}
...
<<scatter_gg_5, fig.width = 5, out.width = "0.35\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length, colour = Species)) +
  geom_point(shape = 16, size = 3) +
...
\begin{columns}
\column[t]{0.48\linewidth}
<<scatter_gg_6a, out.width="0.7\\linewidth", out.height="0.6\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length,
                                  colour = Species)) +
  geom_point(shape = 16, size = 3)
@
\column[t]{0.48\linewidth}
<<scatter_gg_6b, out.width="0.6\\linewidth", out.height="0.6\\linewidth">>=
ggplot(data = iris,
...
\begin{columns}
\column[t]{0.48\linewidth}
<<scatter_gg_7a, out.width="0.7\\linewidth", out.height="0.6\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length,
                                  colour = Species)) +
...
\column[t]{0.48\linewidth}
<<scatter_gg_7b, out.width="0.7\\linewidth", out.height="0.6\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length,
                                  fill = Species)) +
...
<<scatter_gg_8>>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length, fill = Species)) +
  geom_point(shape = 21, size = 3) +
  theme(legend.position = "none") +
  geom_hline(mapping = aes(yintercept = mean(Petal.Length)), lty = 3) +
  geom_vline(mapping = aes(xintercept = mean(Petal.Width)), lty = 2) +
...
<<scatter_gg_9>>=
versicolor <- subset(x = iris, Species == "versicolor")
setosa     <- subset(x = iris, Species == "setosa")
ggplot() +
  geom_point(data = versicolor, mapping = aes(x = Petal.Width, y = Petal.Length), fill = "red", shape = 21, size = 3) +
  geom_point(data = setosa, mapping = aes(x = Petal.Width, y = Petal.Length), fill = "blue", shape = 21, size = 3)
@
\vfill
Note 1: unlike traditional plots, \verb`ggplot` will automatically adjust the axes limits.\\
Note 2: if needed you can define the \texttt{aes()} in the \texttt{geom}s!!
\end{frame}
...
\begin{frame}[fragile, t]{Traditional graphics: scatter plots}
You can add text and arrows:
<<scatter_gg_10, out.height="0.3\\linewidth", out.width="0.35\\linewidth">>=
max.value <- iris[iris$Petal.Length == max(iris$Petal.Length), ]
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length, fill = Species)) +
  geom_point(shape = 21, size = 3) +
  theme(legend.position = "none") +
  geom_segment(data = max.value, aes(x = Petal.Width - 0.75,
                                     xend = Petal.Width,
                                     y = Petal.Length,
                                     yend = Petal.Length),
               size = 1, arrow = arrow(length = unit(0.5, "cm"))) +
  geom_text(data = max.value, mapping = aes(x = Petal.Width - 0.9,
                                            y = Petal.Length, label = "Max"))
@
\end{frame}
...
<<scatter_gg_11, out.height="0.3\\linewidth", out.width="0.35\\linewidth">>=
test_mod <- lm(Petal.Length ~ Petal.Width, data = iris)
newdat   <- data.frame(Petal.Width = seq(0, 3, length.out = 100))
pred     <- predict(object = test_mod, newdata = newdat, interval = "confidence")

ggplot() +
  geom_point(data = iris,    mapping = aes(x = Petal.Width, y = Petal.Length), fill = "grey", shape = 21, size = 3) +
  geom_ribbon(data = cbind(newdat, pred), mapping = aes(x = Petal.Width, ymin = lwr, ymax = upr), fill = "purple", alpha = 0.5) +
  geom_abline(intercept = coef(test_mod)[1], slope = coef(test_mod)[2], lty = 2)
@
\vfill
Note: because we are building the plot with two different datasets (i.e. \verb`iris` \& \verb`newdat`), we specify the \texttt{data} and \texttt{aes()} separately in each line of the ggplot code.
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: scatter plots}
In ggplot, you can also do basic statistics directly:
<<out.height="0.3\\linewidth", out.width="0.35\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length)) +
  geom_point(fill = "grey", shape = 21, size = 3) +
  geom_smooth(method = "lm") +
  geom_smooth(method = "loess", fill = "pink", colour = "red")
@
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: box plots}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: histograms}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: bar plots}
In ggplot, use \verb`geom_col()` to draw a bar plot:
<<bar_gg_1>>=
ggplot(data = spp_means, mapping = aes(x = Species, y = mean)) +
  geom_col()
@
\end{frame}
...
<<aes_gg_1>>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length, fill = Species, size = Sepal.Length)) +
  geom_point(shape = 21)
@
\end{frame}
...
<<aes_gg_2>>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length, fill = Species, alpha = Sepal.Length)) +
...
<<aes_gg_3>>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length, fill = Sepal.Length)) +
  geom_point(shape = 21, size = 5)
@
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Using the aesthetic argument more}
You can indicate values for a third dimension in different ways:
\begin{columns}
\column[t]{0.48\linewidth}
<<aes_gg_4a, out.width="0.8\\linewidth", out.height="0.7\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length,
                                  fill = Sepal.Length)) +
  geom_point(shape = 21, size = 5) +
...
@
\column[t]{0.48\linewidth}
<<aes_gg_4b, out.width="0.8\\linewidth", out.height="0.7\\linewidth">>=
ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length,
                                  size = Sepal.Length)) +
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Combining plots}
Combining plots is less straightforward in \texttt{ggplot2}\dots
\vfill
You need to use an additional package:
<<aes_gg_6, out.width = "0.56\\linewidth", out.height = "0.28\\linewidth", fig.height = 5, fig.width = 10, message = FALSE>>=
scatter <- ggplot(iris, mapping = aes(x = Petal.Width, y = Petal.Length)) + geom_point() +
  labs(x = "Petal Width (cm)", y = "Petal Length (cm)")

box     <- ggplot(iris, mapping = aes(x = Species, y = Petal.Length)) + geom_boxplot()
...
\vfill
\pause
Challenge: have a second look at the relationship between the happiness and the price of present using the dataset \texttt{Surprise}.
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Modifying typefaces}
Unlike traditional graphics, in \texttt{ggplot2} you can easily change the typeface of individual elements in theme:
<<aes_gg_8, out.width = "0.56\\linewidth", out.height = "0.28\\linewidth", fig.height = 5, fig.width = 10>>=
scatter <- ggplot(data = iris, mapping = aes(x = Petal.Width, y = Petal.Length)) +
  geom_point() +
  labs(x = "Petal Width (cm)", y = "Petal Length (cm)") +
  theme(axis.text = element_text(size = 12, colour = "red", family = "serif"), axis.title = element_text(size = 12, family = "mono"))

box     <- ggplot(iris, mapping = aes(x = Species, y = Petal.Length)) +
  geom_boxplot() +
...
<<aes_gg_9, dev='png'>>=
foo <- round(pi, 3)
ggplot(iris, mapping = aes(x = Petal.Width, y = Petal.Length)) +
  geom_point() +
  labs(x = expression(beta^2 + 2[alpha]), y = expression(paste(italic("bla bla bla"))), title = paste("Plot number", foo)) +
  annotate("text", x = c(0.4, 0.5), y = c(4, 4), label = c("\u2640", "\u2642"), size = 12) + ## requires font with unicode installed!
...
\texttt{ggplot2} also has a number of preset themes that you can use:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{ggplot2}: Exporting}
<<tidy=F, eval=F>>=
?ggsave
@
\vspace{0.5cm}
...
\begin{frame}[fragile]{There are other \texttt{geom} out there!}
There are a few other plot types in \texttt{ggplot2} that are worth looking at (check the cheatsheet!):
\begin{itemize}
\item \verb`geom_violin()`, \verb`geom_area()`, \verb`geom_smooth()`, \verb`geom_density()`
\end{itemize}
\vfill
\pause
But the true force of having developed something as modular as \texttt{ggplot2} is that there are many more packages each week providing \texttt{ggplot2} extensions!
\vfill
A few random examples of \texttt{ggplot2} extensions:
\begin{itemize}
\item \verb`ggforce` (for extra geoms to draw circles, pie charts, fancy violin plots, \dots)
...
\item \verb`ggsn` (for adding a scale bar and north arrow to maps)
...
\end{itemize}
\vfill
Note: see also \url{http://www.ggplot2-exts.org/gallery/} (but the list is incomplete).
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{A random example: \texttt{ggmap}}
<<message = FALSE>>=
library(ggmap)

map_us <- get_map(location = c(lon = 13.349342, lat = 52.500033), source = "google", maptype = "satellite", zoom = 20)

ggmap(map_us, extent = "panel") +
  geom_point(mapping = aes(x = 13.349342, y = 52.500033), colour = "white", size = 60, shape = 21) +
  geom_text(mapping = aes(x = 13.349342, y = 52.500033), label = "ALEX IS\n HERE", colour = "white", size = 8) +
  labs(x = "longitude", y = "lattitude")
@
\vfill
Note: this package does not seem very well maintained\dots
\end{frame}
...
\begin{frame}{Challenge}
\begin{center}
\large{Use the datasets called \texttt{population\_UK} and \texttt{deaths\_UK} to compute yearly mortality rates for individuals \underline{below 15 yrs only} and make a nice plot of the outcome using \texttt{ggplot2}!}
\end{center}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Simple solution}
<<>>=
ggplot(data = data_processed, mapping = aes(y = rel_Deaths, x = Year)) +
  geom_area()
@
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{More complex solution}
<<dev='cairo_pdf', echo = FALSE>>=
ggplot(data = data_processed, mapping = aes(y = rel_Deaths, x = Year)) +
  geom_area(fill = "darkgreen") +
  ylab("Mortality rate (\u2030)") +
  labs(title = paste0("Children mortality (", min(data_processed$Year), "-", max(data_processed$Year), ")")) +
  scale_x_continuous(breaks = seq(min(data_processed$Year), max(data_processed$Year), by = 5),
                     minor_breaks = min(data_processed$Year):max(data_processed$Year)) +
  scale_y_continuous(breaks = 0:ceiling(max(data_processed$Year)),
                     minor_breaks = NULL) +
  theme_classic() +
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{More complex solution}
<<eval = FALSE>>=
ggplot(data = data_processed, mapping = aes(y = rel_Deaths, x = Year)) +
  geom_area(fill = "darkgreen") +
  ylab("Mortality rate (\u2030)") +
...
\begin{frame}{Challenge}
\begin{center}
\large{Use the datasets called \texttt{population\_UK} and \texttt{deaths\_UK} to compute yearly mortality rates for individuals \underline{of each age class} and make a quick plot of the outcome using \texttt{ggplot2}!}
\end{center}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Possible solution}
<<out.width="0.55\\linewidth", fig.width=8>>=
deaths_UK %>% group_by(Year, Age_cat) %>% summarize(tot_Deaths = sum(Deaths)) -> deaths_processed_all
population_UK %>% group_by(Year) %>% summarize(tot_Pop = sum(Pop)) -> pop_processed## as before
full_join(deaths_processed_all, pop_processed) %>% mutate(rel_Deaths = 1000*tot_Deaths/tot_Pop) -> data_processed_all

ggplot(data = data_processed_all, mapping = aes(y = rel_Deaths, x = Year, fill = Age_cat)) +
  geom_area()
@
\end{frame}
...
\section{Conclusion: traditional graphics vs. \texttt{ggplot2}}
...
\begin{itemize}
  ...
  \item main restriction: always add things on top of what has already been plotted
\end{itemize}
\end{block}
\pause
\column[t]{0.45\linewidth}
...
\begin{itemize}
  ...
  \item more accessible than lattice
\end{itemize}
\end{block}
\end{columns}
\vfill
\pause
\begin{center}
\large{$\rightarrow$ It may be useful to know both!?}
\end{center}
\end{frame}
