...
\vfill
\pause
Double squared brackets can also be used for subsetting of \underline{single} elements; they drop names (and other attributes):
<<>>=
foo[[1]] + foo[[2]]
foo[["alex"]] + foo[["colin"]]
@
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Vector: coercion}
...
\item find out which date is internally stored as 0? %c(as.Date(x = "2018/06/18"), 0)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Combining vectors with different levels}
...
\vfill
\pause
Note: if you want more modern functions to manipulate factors, look at the package \texttt{forcats}.
\end{frame}
...
\vfill
\pause
Note 2: if you want more modern functions more consistent than the \texttt{*apply()} ones, look at the package \texttt{purrr}.
\vfill
\pause
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Data frames: challenge}
The \texttt{iris} data set (\url{https://en.wikipedia.org/wiki/Iris_flower_data_set}):
\vfill
\begin{figure}
    \centering
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Working directory}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Exporting and importing data in the \R \ binary format}
\r \ can write and read binary formats that take by convention the extensions \texttt{.rda} or \texttt{.RData}.
\vfill
Example:
<<>>=
my_iris <- iris
...
\begin{block}{}
\begin{itemize}
\item \r \ cannot read/write \texttt{.xls(x)} files out of the box\\
\item Several packages can do that, but it is safer to use \texttt{.csv} or \texttt{.txt} files\\
\item Excel can read and write \texttt{.csv} \& \texttt{.txt} files!\\
\end{itemize}
\end{block}
\end{minipage}
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Meeting the \texttt{tidyverse} world}
Two different ways to solve the problem: \emph{what is the average length and width for each iris species?}
\vfill
Standard \r \ approach:
<<>>=
tapply(X = iris$Sepal.Length, INDEX = iris$Species, FUN = mean)
...
\vfill
\pause
The same operation using the package \texttt{dplyr} from \texttt{tidyverse}:
<<message = FALSE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Some words about the \texttt{tidyverse}}
The \texttt{tidyverse} packages (\url{https://www.tidyverse.org/}) are developped by RStudio:
...
\begin{columns}[t]
\column{0.45\linewidth}
\begin{block}{\R \ is from the \R \ core team who}
\begin{itemize}
...
\end{block}
\column{0.45\linewidth}
\begin{block}{\texttt{tidyverse} is from the RStudio people who}
\begin{itemize}
...
\item $\sim$ 80 employees + tons of volunteers% https://www.rstudio.com/about/
...
Note 2: more and more users rely on \texttt{tidyverse} and many recent packages use the \texttt{tidyverse} grammar.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Getting started with \texttt{tidyverse}}
\begin{center}
\includegraphics[height = 0.8\textheight]{../figures/Wickham_r4ds}
\end{center}
\vfill
Note: there are also multiple tutorials on the web\\ (e.g. \url{https://www.r-bloggers.com/lesser-known-dplyr-tricks/}).
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Key \texttt{tidyverse} principles}
\begin{itemize}
\item one verb = one action = one function
\item operations can be chained with the pipe operator ``\texttt{\%>\%}'' (from package \texttt{magrittr}), which considers the output from one function as the input of the next function (exception: \texttt{ggplot2} that uses ``\texttt{+}'' instead)
\item the outcome of a given function is always of the same \texttt{class}
\end{itemize}
\vfill
\pause
\begin{columns}[t]
...
\item lead to bad habits (e.g. help not looked at)
\item wider gap between users and developers
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{dplyr} verbs}
...
\vfill
\pause
\item create and keep only new columns with \texttt{transmute()}
<<edit df dplyr2>>=
iris %>%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}[fragile, t]{Around \texttt{dplyr} verbs}
% These main \texttt{dplyr} functions have derivatives and some of them can be useful:\\
...
% str(data.frame(iris %>% mutate_if(is.numeric, ~ as.character(.))))
% @
% \end{columns}
% \vfill
% you do:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{What are tibbles?}
Tibbles are modified data frames that can be used and (sometimes automatically) produced by \texttt{tidyverse} packages:
<<>>=
iris %>%
  group_by(Species) %>%
...
@
\vfill
Note: most of what works on objects of class \texttt{data.frame} works on objects of class \texttt{tbl} (but not all as they don't consider row names).
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{From tibbles to data frames and back}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{How to influence the display of tibbles?}
...
\vfill
\pause
Note: to always display all row you can set \texttt{options(dplyr.print\_min = Inf)}.
\end{frame}
...
\subsection{advanced but useful features}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{group\_by()}}
The \texttt{group\_by()} function allows you to perform operation on grouped data.\\
\vfill
\pause
...
\vfill
\pause
\item \texttt{do()} $\rightarrow$ for applying custom functions on each "group"\\ (but advanced and perhaps soon deprecated)
\end{itemize}
\vfill
\end{frame}
...
\begin{itemize}
\item 1915% population_UK %>% group_by(Year) %>% summarize(sum(Pop)) %>% filter(Year == 1915)
\item 2015
...
%  filter(Year >= 1915 & Year <=2015) %>%
%  select(Pop) %>%
%% pull
\end{itemize}
\vfill
Use the dataset called \texttt{deaths\_UK} (careful, quite large!) and figure out:
\begin{itemize}
...
\item which were the top 3 detailed causes of death before 1930 for each of the 8 broader categories\\
(most difficult)
\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{\texttt{group\_by()} with \texttt{do()}}
Advanced: you want to apply a function that returns several elements, such as the range of \texttt{Petal.Length} for each species of iris:
<<>>=
iris %>%
  group_by(Species) %>%
...
\vfill
\pause
More sophisticated alternative (combining \texttt{tidyverse} and standard \r):
<<>>=
...
@
%#true dplyr + purrr alternative
%iris %>%
%  group_by(Species) %>%
%  nest() %>%
%  mutate(output = map(data, function(x){
%
%    bind_rows(Range(x$Petal.Length))
%
%  })) %>%
%  unnest(output, .drop = T) %>%
%  data.frame()
\vfill
\pause
% Alternatives:
% iris %>%
%   group_by(Species) %>%
%   do(tidy(Range(.$Petal.Length))) %>%
%   spread(key = names, value = x) %>% ## spread the lists
%   select(Species, min, max) %>% ## reorder columns
%   data.frame()
%
% iris %>%
%   split(.$Species) %>%
%   map_df(., function(df) c(as.character(df$Species[1]), range(df$Petal.Length))) %>%
%   t %>%
%   data.frame(., row.names = 1:nrow(.)) %>%
%   rename(Species = V1, min = V2, max = V3)
Alternatively, stick to standard \r \ and the function \texttt{tapply()}.
%list_range <- with(data = iris, tapply(X = Petal.Length, INDEX = Species, FUN = Range))
%data.frame(Species = levels(iris$Species), do.call(what = "rbind", args = list_range), %row.names = NULL)
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Using \texttt{dplyr} to merge datasets}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{frame}[fragile]{Using \texttt{dplyr} to merge datasets}
%You can also use \texttt{left\_join()} to keep all data from the first dataset and keep only the new data from the second %data set that fit in:
%\vfill
%<<left join>>=
%(flower_quality <- data.frame(Species = c("setosa", "versicolor", "virginica"),
%                              Quality = c("High", "Medium", "Low")))
%@
%\pause
%<<>>=
%left_join(my_df3, flower_quality, by = "Species")
%@
%\pause
%<<>>=
%full_join(my_df3, flower_quality, by = "Species")
%@
%\end{frame}
...
\begin{frame}[fragile]{Challenge}
\begin{center}
\large{Use the datasets called \texttt{population\_UK} and \texttt{deaths\_UK} to compute yearly mortality rates for individuals below 15 yrs only.}
\end{center}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Reshaping data frame}
...
\item \texttt{spread()} turns long data into wide (useless?)
\end{itemize}
\end{frame}
...
iris[c(1:5, 51:55), ] %>% select(Species, Sepal.Length) %>% mutate(day = paste0("day_", rep(1:5, 2))) %>% spread(., day, Sepal.Length) -> iris_wide
iris_wide %>% gather("day", "Sepal.Length", -Species) %>% separate(day, c("obs", "day")) %>% select(-obs) %>% mutate(day = as.numeric(day)) %>% arrange(Species, day) -> iris_long
#save(iris_wide, file = "./data/iris_wide.rda", compress = "xz")
#save(iris_long, file = "./data/iris_long.rda", compress = "xz")
@
\vfill
\begin{columns}[t]
...
iris_wide ## note: stored in BeginR!
@
\vfill
you want:
<< reshaping df2, echo = T>>=
iris_long ## note: stored in BeginR!
@
\vfill
\pause
