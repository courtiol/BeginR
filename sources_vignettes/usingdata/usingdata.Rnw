...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{Matrices: general properties}
In the background, a matrix is a vector with dimension attributes (i.e. number of columns and rows).%
...
\vfill
\pause
You can subset the data using this dimension attributes or not:
<<>>=
my_matrix[5]
my_matrix[2, 2]
@
\vfill
\pause
A matrix will be filled from top - bottom, left - right:
<<>>=
(my_matrix4 <- matrix(data = 1:4, ncol = 2, nrow = 2)) ## but try byrow = TRUE
@
\end{frame}
...
They borrow from both matrices and lists:
\vfill
\begin{columns}[t]
\column{0.45\linewidth}
As for matrices:
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \caption*{\emph{Iris setosa}\\ \textcopyright Miya.m}
        \end{center}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        \begin{center}
        \includegraphics[height=3cm]{"../figures/Iris_versicolor"}
        \caption*{\emph{Iris versicolor}\\ \textcopyright D.G.E. Robertson}
        \end{center}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        \begin{center}
        \includegraphics[height=3cm]{"../figures/Iris_virginica"}
        \caption*{\emph{Iris virginica}\\ \textcopyright F. Mayfield}
        \end{center}
    \end{subfigure}
\end{figure}
\vfill
...
The same operation in \texttt{dplyr} looks very different:
\vfill
<<>>=
iris %>%
  group_by(Species) %>%
...
\vfill
\pause
%Range <- function(x, ...) c(min = min(x, ...), max = max(x, ...)) ## same as range() but with names!
%iris %>%
%   group_by(Species) %>%
...
<<>>=
head(as.data.frame(iris_tbl))
@
\vfill
\pause
<<>>=
as_tibble(as.data.frame(iris_tbl))  ## no need for head as tibble!
@
\vfill
\pause
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{How to influence the display of tibbles?}
Sometimes the default is a little too dangerous!
\vfill
<<>>=
foo <- c(178, 175, 159, 164, 183, 160, 181, 189, 174, 177)
level <- c(rep("A", 6), rep("B", 4))
@
\vfill
<<>>=
d <- data.frame(foo, level)
d %>%
     group_by(level) %>%
     summarize(Mean = mean(foo))
@
\vfill
\pause
Compare to base \r:
<<>>=
c(mean(foo[level == "A"]), mean(foo[level == "B"]))
@
\vfill
Note: but at least they have recently added the "." suggesting there are more number out there!
\end{frame}
...
my_iris <- read.csv("my_iris.csv") ## or read.table() with adequate options!
head(my_iris)
@
<<echo = FALSE>>=
