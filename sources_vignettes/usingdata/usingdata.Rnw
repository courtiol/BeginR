...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile, t]{The \texttt{dplyr} alternative to \texttt{tapply()}}
The same operation using the package \texttt{dplyr} looks very different:
\vfill
<<message = FALSE>>=
library(dplyr)
...
\end{center}
\vfill
Note: there are also mutliple tutorial on the web\\ (e.g. \url{https://www.r-bloggers.com/lesser-known-dplyr-tricks/}).
\end{frame}
...
\item gap between users and developers
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
Tibbles are modified data frames that are automatically created by \texttt{tidyverse} packages:
<<>>=
iris_tbl <- iris %>%
  group_by(Species) %>%
...
\vfill
\pause
\item \texttt{do()} $\rightarrow$ for applying custom functions on each "group"
\end{itemize}
\vfill
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Example of correction}
...
deaths_UK %>%
  filter(Year <= 1930) %>%
  group_by(Desc) %>%
  summarize(tot_deaths = sum(Deaths), Cause_simple = Cause_simple[1]) %>%
  arrange(Cause_simple) %>%
...
iris %>%
  group_by(Species) %>%
  do(data.frame(as.list(Range(.$Petal.Length)))) %>%  ## for this specific example we could of course use summarize with min and max but we want something that will work no matter the function!
  data.frame()
@
\vfill
\pause
