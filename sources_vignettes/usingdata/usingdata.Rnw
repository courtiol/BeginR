...
iris[c(1:5, 51:55), ] %>% select(Species, Sepal.Length) %>% mutate(day = paste0("day_", rep(1:5, 2))) %>% spread(., day, Sepal.Length) -> my_df1
my_df1 %>% gather("day", "Sepal.Length", -Species) %>% separate(day, c("obs", "day")) %>% select(-obs) %>% mutate(day = as.numeric(day)) %>% arrange(Species, day) -> my_df2
@
\vfill
\begin{columns}[t]
