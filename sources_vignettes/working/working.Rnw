...
\item \texttt{clang} and \texttt{gfortran} (\url{https://cran.r-project.org/bin/macosx/tools})\\ or the large \texttt{Xcode} (\url{https://developer.apple.com/xcode}) if you use macOS
...
\begin{frame}{Practice}
\begin{enumerate}
\item create a separate \r \ markdown file that you will knit every morning to update \texttt{BeginR}
\item knit the \r \ markdown file to install the package for the first time
\end{enumerate}
\end{frame}
\selectmanualcolour{blue!75!black}
...
Notes:
\begin{itemize}
\item you can also use RStudio to load a package but that would not stay in your \r \ makrdown file, so I do not recommend doing that
\pause
\item you can check the (exported) content of a package:
<<eval = FALSE>>=
library(help = "BeginR")
@
\end{itemize}
...
Notes:
\begin{itemize}
\item some packages can help to do this: \verb+InstallR+ on Windows and \verb+UpdateR+ on macOS
\item  also update RStudio for full compatibility with \r \ (\texttt{Help $\rightarrow$ Check for Updates})
\end{itemize}
\end{frame}
...
\begin{center}
\begin{figure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/Wickham_r4ds}
\caption*{$\sim 30$ \euro \ (or 0\euro)}
\end{subfigure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/bookBecksi}
\caption*{$\sim 30$ \euro}
\end{subfigure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/Rbook2}
\caption*{$\sim 35$ \euro}
\end{subfigure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/Dalgaard_book}
\caption*{$\sim 40$ \euro}
\end{subfigure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/Rbook}
\caption*{$\sim 60$ \euro}
\end{subfigure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/Wickham_AdvancedR}
\caption*{$\sim 60$ \euro \ (or 0\euro)}
\end{subfigure}
\begin{subfigure}[b]{1.8cm}
\centering
\includegraphics[height=2.5cm]{../figures/Chambers_book1}
\caption*{$\sim 90$ \euro}
\end{subfigure}
\end{figure}
\end{center}
...
\item many others (e.g. Journal of Open Source Software, Methods in Ecology \& Evolution)
\end{itemize}
\end{frame}
...
Conferences:
\begin{itemize}
\item useR (\url{https://user2019.r-project.org})
...
\begin{center}
\huge{\huge{After having learned some basics, just open RStudio and test your understanding by performing experiments!}}
\vfill
