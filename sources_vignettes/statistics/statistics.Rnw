...
\vfill
\pause
Note: these z-tests are assymptotic tests relying on normality of parameter estimates. They are not to be trusted with small dataset for GLM! There is no easy alternative, so better trust the likelihood ratio test of the overall predictor!
\end{frame}
...
Liner mixed-effects models (LMM) and generalised linear mixed-effects models (GLMM) are extensions from, respectivelly, LM and GLMM for situation for which there is covariation between the errors.
\vfill
\pause
...
@
\vfill
Note: the model is a little slow to fit\dots
\end{frame}
