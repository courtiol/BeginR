...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t, fragile]{\r \ provides many statistical tests out of the box}
Many (not all) tests allow for the use of a formula-based syntax:
\vfill
<<>>=
...
\vfill
\pause
Note: linear models for a more sofisticated parametric alternative (see later).
\end{frame}
...
\vfill
\pause
Note: linear mixed-effects models for parametric alternative.
\end{frame}
...
<<>>=
prop.test(x = cbind(success = c(8, 4), failure = c(2, 6)))
@
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Packages provide even more simple statistical tests!}
Some examples:
\begin{itemize}
...
\r \ primary goal is to perform statistical analyses. So among the many thousands of
packages many focus on statistical tools and by no means I will try to cover or even
summarise this diversity. I have chosen to only illustrate some of the tools I know and that I have used to show you how to do some statistics in \r. Since time is limited, I will not for example illustrate any Bayesian methods, nor machine learning methods, although some good packages exist for that too!
\end{frame}
...
<<>>=
pca_US <- prcomp(~ Murder + Assault + Rape, data = USArrests, scale. = TRUE)
biplot(x = pca_US)
@
\end{frame}
...
Clinton_state <- c("California", "Colorado", "Connecticut", "Delaware", "Hawaii",
