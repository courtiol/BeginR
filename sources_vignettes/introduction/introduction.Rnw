...
I will:
\begin{itemize}
\item give you all the slide (so write only what is not being displayed)
\item explain the main principles of the \r \ language, so to give you good basics
\item provide you with short examples on how to use some of the most useful functions in \r
\item sometimes show you different ways of doing the same things
...
\item focus on the big picture (try to understand the logic, consider the long term gains)
\item ask any silly question that pops up in your creative mind
\end{itemize}
\end{frame}
...
\vfill
  \begin{itemize}
  \item 2003 -- : studying \r \ (still ongoing)
  \vfill
  \item 2008 -- : using \r \ most days
  \vfill
  \item 2010 -- : teaching \r
  \vfill
  \item 2013 -- : debugging \r \ packages
  \vfill
  \item 2016 -- : developing \r \ packages
  \vfill
  \item 2018 -- : debugging \r
  \end{itemize}
\end{itemize}
\end{frame}
...
\item 05/12/1997: \r \ version 0.6 is part of GNU project (``freedom to share, freedom to chang'')
...
\item write a script and comment it properly
\item make sure your script always work
\item store each result as an object with a useful name
\end{enumerate}
\end{frame}
...
\begin{itemize}
\item the help files: every single (exported) function has an help file associated to it!
\item official manuals (boring but very thorough \url{https://cran.r-project.org/manuals.html})
\end{itemize}
\vfill
\pause
Books:
...
\includegraphics[height=3.5cm]{../figures/book}
...
\end{center}
\vfill
Note: there are many cheatsheets covering many aspects of \r \ and several \texttt{tidyverse} packages!
\end{frame}
...
\item PR statistics (\url{https://www.prstatistics.com})
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{The best person who can teach you? YOU!}
\begin{center}
